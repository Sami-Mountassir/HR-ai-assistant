from database_management import SessionLocal
from Employees import Employees
from Teams import Teams
from Departments import Departments  # fixed typo: was "Departements"
...
def get_team_by_employee(name: str):
    """Return the Team object for the employee with the given name."""
    db = SessionLocal()
    try:
...
def department(name: str):
    employee = get_employee(name)
    # Returns the department name string, not the ORM object
...
def work_experience(name: str):
    employee = get_employee(name)
    return employee.experience_years if employee else None  # column is experience_years


def attendance_issues(name: str):
    """Returns the attendance float score. Low values signal issues."""
    employee = get_employee(name)
    return employee.attendance if employee else None


# ─── Team field accessors (looked up via the employee's name) ─────────────────
...
def team_members(name: str):
    """Return the list of Employees ORM objects that belong to this employee's team."""
    team = get_team_by_employee(name)
    return team.members if team else []


# ─── Company-wide / financial helpers ─────────────────────────────────────────

def financial_status() -> str:
    """Return a summary of every team's income vs outcome."""
    db = SessionLocal()
    try:
        ...
        lines = [
            f"- {t.name}: income {t.income}, outcome {t.outcome}"
            for t in teams
        ]
        return "\n".join(lines)
    finally:
...
def high_risk_departments() -> str:
    """Return the 3 least profitable departments."""
    db = SessionLocal()
    try:
        ...
        return (
            f"Most endangered department is {deps[0].name}, "
...
def company_database() -> str:
    """Return a plain-text table of all employees for use in AI prompts."""
    db = SessionLocal()
    try:
        ...
        header = (
            "Name | Role | Department | Performance | Leadership | "
        ...
        for e in employees:
            ...
            rows.append(
                f"{e.name} | {e.role} | {dep_name} | {e.performance_rate} | "
